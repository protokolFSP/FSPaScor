...
We assume:
- Phase 1: anamnesis/interview (assistant asks questions) ~ first 20 minutes (fixed).
- Phase 2: presentation (assistant presents patient) starts after anamnesis, length varies (auto-detected).
- Phase 3: feedback/rückmeldung (often Oberarzt) near end (auto-detected, excluded from final score).
...
def detect_presentation_start(
    ...
    """
    Finds first time after after_s where rolling avg report-score exceeds threshold.
    """
...
def detect_feedback_start(
    ...
    """
    Searches in the last lookback_s seconds for rolling avg feedback-score.
    Returns earliest time in that region crossing threshold.
    """
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument("--max_new_files", type=int, default=2)
    ap.add_argument("--clip_seconds", type=int, default=4200)  # 70 min safe default
    ap.add_argument("--anamnesis_seconds", type=int, default=1200)  # fixed 20 min
    ...
    ap.add_argument("--feedback_lookback_s", type=float, default=900.0)  # last 15 min
...
# ================================
# file: .github/workflows/update-ia-scores.yml (only the relevant step/env)
# ================================
"""
Set these envs in your workflow for stability on GitHub hosted runner:

CLIP_SECONDS: "4200"        # 70 min (covers 45min presentation + feedback)
MAX_NEW_FILES: "1" or "2"   # keep runtime safe
ANAMNESIS_SECONDS: "1200"   # fixed 20 min
PHASE1_WEIGHT / PHASE2_WEIGHT as you want

Then call:
python -u tools/update_scores_from_ia.py ... --clip_seconds "${CLIP_SECONDS}" --anamnesis_seconds "${ANAMNESIS_SECONDS}" ...
"""
