...
def count_fillers(words: List[str], cfg: ScoringConfig) -> int:
    fillers = set(t.lower() for t in cfg.filler_tokens)
    return sum(1 for w in words if w in fillers)


def count_repetitions(words: List[str]) -> int:
    """
    Simple repetition: count tokens repeated within a short window.
    """
    rep = 0
    window = 3
...
def count_weird_tokens(raw_text: str) -> int:
    """
    Count suspicious artifacts (encoding issues, very long tokens, digits/underscores, repeated chars).
    """
...
def compute_text_metrics(assistant_text: str, cfg: ScoringConfig, long_silence_sec_per_min: float) -> TextMetrics:
    ...
    filler = count_fillers(words, cfg)
    rep = count_repetitions(words)
    weird = count_weird_tokens(assistant_text)

    filler100 = _per_100(filler, wc)
    rep100 = _per_100(rep, wc)
    weird100 = _per_100(weird, wc)

    # Soft penalties with log1p (not harsh)
    clarity_pen = (
